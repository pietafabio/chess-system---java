...
public class Board {
	...
	public Board(int rows, int columns) {
		...
		pieces = new Piece[rows][columns]; // instanciação da matriz 'pieces' com 'Piece' na quantidade de linhas e
											// colunas informadas
	}
	...
	public Piece piece(int row, int column) { // criado método para retornar a posição de uma peça após informada
												// posição de linha e coluna retornando um objeto do tipo 'Piece', nome
												// do método 'piece', recebendo 'int row' e 'int column'
	...
	public void placePiece(Piece piece, Position position) { // método: 'placePiece' recebendo uma 'piece' e uma
																// 'position' na matriz 'pieces' na linha
																// 'position.getRow' e na linha 'position.getColumn',
																// atribuir a esta posição da matriz ´pieces' a peça que
																// veio como argumento (linha de cima) e já declarada no
																// 'Board', linha 7 e instanciada no costrutor, linha
																// 12.
	...
	private boolean positionExists(int row, int column) { // quando uma posição existe? 'row' menor ou igual a 0' e
															// 'row' menor do que a altura do 'board' ('rows') column
															// maior ou igual a '0' e menor do que a quantidade de
															// 'columns'
